...
The carewright operator manages the CPG Ingester and Care Plan Writer pipelines.
Pipeline components are represented as OpenShell `SandboxRequest` resources so
they run as policy-controlled sandboxes rather than ordinary Deployments. The
pipeline orchestration itself is represented as a `SonataFlow` resource and is
run by the separately installed SonataFlow Operator.
...
Once OpenShell has generated an HTTP URL for every component, each pipeline
controller creates an owned `sonataflow.org/v1alpha08` `SonataFlow` resource
and the matching `<workflow-name>-props` ConfigMap. Component URLs in the
workflow and callback URLs sent to the sandboxes are generated by the operator.
The component `SandboxRequest` policies include the workflow Service as an
allowed egress endpoint, and the BFF receives its generated `SONATAFLOW_URL`.
The pipeline reports Ready only when every sandbox is current and SonataFlow's
`Running` condition is true.

Install the SonataFlow Operator before enabling the `cpgingester` or
`careplanwriter` controllers. A namespace-wide `SonataFlowPlatform`, when
needed for the chosen profile and supporting services, remains shared
infrastructure and is not owned by an individual pipeline.
...
`spec.networkAccess` adds typed egress destinations to the effective OpenShell
policy. Pipeline controllers use it to permit calls to their generated
SonataFlow Service without requiring users to edit policy YAML with an
operator-generated hostname.
...
```sh
make docker-build docker-push IMG=<some-registry>/carewright-operator:tag
```
...
```sh
make deploy IMG=<some-registry>/carewright-operator:tag
```
...
```sh
make build-installer IMG=<some-registry>/carewright-operator:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/carewright-operator/<tag or branch>/dist/install.yaml
```
